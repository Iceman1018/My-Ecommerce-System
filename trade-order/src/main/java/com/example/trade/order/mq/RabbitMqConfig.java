...
public class RabbitMqConfig {
     ...
     */
    @Bean
    public Queue SeckillReleaseQueue(){
        Queue queue = new Queue("seckill.order.revert.queue", true, false, false);
        return queue;
    }

    /**
     * 用于seckill与order通信，获得order支付成功消息
     *
     * @return
     */
    @Bean
    ...
    public Binding seckillReleaseBinding() {
        return new Binding("seckill.order.revert.queue",
                Binding.DestinationType.QUEUE,
                "order-event-exchange",
                "seckill.order.revert",
                null);
    }
